...
const PinDetail = ({ user }) => {
  ...
  if(!pinDetail) return <InfinitySpin message="Cargando..."/>
  ...
  return (
    ...
    <div className="flex xl-flex-row flex-col m-auto bg-white" style={{ maxWidth:'1500px', borderRadius: '32px' }}>
      ...
      <div className="w-full p-5 flex-1 xl:min-w-620">
        ...
        <div>
          <h1 className="capitalize mt-10">
            Estas son tus skills!!!
          </h1>
          ...
          <div className="max-h-370 overflow-y-auto">
            {pinDetail?.comments?.map((comment, i) => (
              <div className="flex gap-2 mt-5 items-center bg-white rounded-lg" key={i}>
                <img src={comment.postedBy.image} alt="perfil de usuario" 
          ...
          <Link to={`user-profile/${pinDetail.postedBy?._id}`} >
            ...
            />           
              ...
              <button 
                ...
                onClick={addComment}
                >
                  {addingComment ? 'Agregando comentario...' : 'Enviar!'}
              </button>
          </div>
      </div>
    </div>
    ...
    {pins?.length > 0 ? (
      ...
      <InfinitySpin message="Cargando mas Hojas..."/>
    )}
    </>
  )
}
